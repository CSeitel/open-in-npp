...
export async function whenFileInfoRead_( ü_fileUri:TFileUri ):Promise<FileStat|null> {
    ...
    if (   ü_info      !== null
      && ( ü_info.type === 66 as any
        || ü_info.type === FileType.SymbolicLink ) ) {
        const ü_real = await ßß_fs_p.realpath( ß_fileToUri( ü_fileUri ).fsPath );
           return await whenFileInfoRead_( ü_real );
         }
    return ü_info;
}
...
export async function whenFileTypeKnown( ü_fileUri:TFileUri ):Promise<TFileType> {
    const ü_info = await whenFileInfoRead( ü_fileUri );
  //
    if ( ü_info      === null     ) { return FileType.Unknown     ; }
    if ( ü_info.type === 66 as 64 ) { return FileType.SymbolicLink; }
                                      return ü_info.type;
}
